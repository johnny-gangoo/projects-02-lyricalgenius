...
class Navbar extends React.Component {
  ...
  componentDidMount(){
    ...
    if(!token){ //lyricalgenius user token
        ...
        axios.post("https://favoriteinvestment.com" + "/verify", token).then(res => {
            if (res.data == "Invalid") {
                window.location.href = './';
            }
            else{
  ...
  handleSubmit = (event) => {
    if (this.state.title !== "" || this.state.name !== "") {
      ...
      axios.post("https://favoriteinvestment.com" + "/getSong", this.state).then(res => {
        res = res.data;
  ...
  handleSongPreview = (songObj) => {
    ...
    songObj.map(async (songObj, index) => (
      await axios.post("https://favoriteinvestment.com" + "/getPreview", songObj).then(res => {
        ...
        <div>
          <nav class="navbar navbar-dark navbar-expand-lg ournavbar">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
              ...
              <FontAwesomeIcon size= '2x' icon={faSignOutAlt} className="img-fluid" style={{"color": "white","cursor": "pointer"}} onClick={() => {let token = getFromStorage("lgut"); axios.post("https://favoriteinvestment.com/logout", {token: token}).then(res => {window.location.href = "./";}).catch(error => {console.log(error)})}} />
            </div>
          </nav>
